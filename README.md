...
- SharePointへのファイル保存
- 紙の名簿とSalesforceの顧客担当者・取引先担当者の照合

### 名簿照合の旧字体・異体字対応

「山崎／山﨑」「斉藤／斎藤／齊藤／齋藤」「高橋／髙橋」「渡辺／渡邊／渡邉」などは、どちらの表記からでも候補を検索します。姓だけでなく、名の旧字体（徳／德、真／眞など）、Unicode互換漢字、異体字セレクタ（IVS）にも対応します。原本とSalesforceの氏名表記は変更しません。

対応表は `backend/src/salesforce/japanese-name.ts` の `NAME_VARIANT_GROUPS` で管理します。すべての外字や読み方を網羅するものではありません。同音だけの別姓（加藤／加東など）は同一視せず、フリガナだけで同一人物とは判定しません。照合結果は従来どおり候補の一覧であり、同姓同名の本人確認は必要です。

候補取得後にも正規化した氏名を比較し、検索用ワイルドカードで拾った無関係な名前を除外します。[Salesforceのページ取得](https://developer.salesforce.com/docs/platform/api-rest/guide/resources-query.html)を利用して、該当候補を各マスタ最大10件まで返します。名簿内の重複警告にも異体字を反映しますが、自動削除の基準は従来の全項目一致のままです。

外部サービスを呼ばない回帰テスト:

```bash
npm -w backend run test:member-check
```
